...
// 根节点的标题文字
#define SLM_ROOT_MENU_TEXT "smart-guide"
// 锁定时提示框标题长度
...
#define SLM_STRING_VAL_SHOW_DEFAULT "%s"
#define SLM_STRING_VAL_SHOW_VAL_LEN 12
...
#else
#    define SLM_LOG_INFO(_TEXT) \
        do {                    \
            (void)0;            \
        } while (0)
#    define SLM_LOG_WARN(_TEXT) \
        do {                    \
            (void)0;            \
        } while (0)
#    define SLM_LOG_ERR(_TEXT) \
        do {                   \
            (void)0;           \
        } while (0)
#    define SLM_PRINTF(...) \
