...
inline void sBSP_UART_Motor_SendByte(uint8_t byte)
{
    HAL_UART_Transmit(&uart3, &byte, 1, uart3_blocking_ms);
}

inline void sBSP_UART_Motor_SendBytes(uint8_t* pData, uint16_t length)
{
...
void sBSP_UART_Motor_RecvBegin(sBSP_UART_RecvEndCb_t recv_cb)
{
    ...
    if (HAL_UARTEx_ReceiveToIdle_DMA(&uart3, (uint8_t*)uart3_recv_buf, sizeof(uart3_recv_buf)) != HAL_OK)
    {
        sBSP_UART_Debug_Printf("串口3:空闲中断DMA接收出错");
    }
}

// IMU UART3初始化已禁用 - UART3现在用于步进电机
/*
...
// IMU Printf和接收函数已禁用 - UART3现在用于步进电机
/*
